...
export default function IAPProducts() {
  ...
  const fetchProducts = async (networkName: string): Promise<Category[]> => {
    ...
    try {
      ...
      console.log("Products loaded and enhanced with network prices:", enhancedCategories);
  ...
  return (
    <main class="container mx-auto p-4 max-w-6xl">
      <PageHeader title="Nine Chronicles IAP Products">
        <p class="text-gray-100">Currently browsing products on the <span class="font-semibold bg-white/20 px-3 py-1 rounded-full">{network()}</span> network</p>
      </PageHeader>
      ...
      <Show when={categories() && categories()!.length === 0 && !categories.loading}>
        <EmptyState 
