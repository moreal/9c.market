 ...
 */
export async function fetchCryptoPrice(symbol: string, currency: string): Promise<number | null> {
  try {
    ...
    const response = await fetch(`https://coinprice-api.9c.market/api/coins/${symbol}/price?currency=${currency}`);
